...
func Test_EdgeCases_GetTypeConsistentValue(t *testing.T) {
	tests := [...]struct {
		...
		{desc: "succeed: from VAR_MAX", input: Variant{VariantType: VAR_MAX, VariantValue: nil}, expectedError: fmt.Errorf("dev error: unhandled variant type")},
	}
...
func TestToDebugString(t *testing.T) {
	...
	tests := [...]struct {
		...
		{desc: "VAR_ERROR", input: Variant{VariantType: VAR_ERROR, VariantValue: fmt.Errorf("Blech")}, expectedValue: "Blech"},
...
func TestCoerceToBool(t *testing.T) {
	...
	tests := [...]struct {
...

		{
			...
			expectedError: (&Variant{VariantType: VAR_STRING}).buildTypeError(targetType),
		},
		{
			desc:          "fail: inconsistent type",
			input:         Variant{VariantType: targetType, VariantValue: "Some Random String"},
			expectedError: (&Variant{VariantType: targetType, VariantValue: "Some Random String"}).buildInconsistentTypeError(),
		},
	}
...
func TestGetStringValue(t *testing.T) {
	...
	tests := [...]struct {
		...
		{desc: "succeed: from string", input: Variant{VariantType: VAR_STRING, VariantValue: sentinel}, expectedValue: sentinel},
		{
			...
			expectedError: (&Variant{VariantType: VAR_DATE}).buildTypeError(targetType),
		},
		{
			desc:          "fail: inconsistent type",
			input:         Variant{VariantType: targetType, VariantValue: 87},
			expectedError: (&Variant{VariantType: targetType, VariantValue: 87}).buildInconsistentTypeError(),
		},
	}

	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			if actualValue, actualError := test.input.GetStringValue(); actualError == nil {
				assert.Equal(t, test.expectedValue, actualValue, "fail")
			} else {
				assert.EqualError(t, test.expectedError, actualError.Error(), actualError.Error())
			}
...
func TestGetErrorValue(t *testing.T) {
	sentinel := fmt.Errorf("Abandon hope")
	...
	tests := [...]struct {
		...
		{desc: "succeed: from error", input: Variant{VariantType: VAR_ERROR, VariantValue: sentinel}, expectedValue: sentinel},
		{
			desc:          "fail: from date",
			input:         Variant{VariantType: VAR_DATE, VariantValue: time.Now()},
			expectedError: (&Variant{VariantType: VAR_DATE}).buildTypeError(targetType),
